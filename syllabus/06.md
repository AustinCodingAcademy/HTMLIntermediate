...
We call this "inheritance". It allows us to build new data build modified data types based off of existing ones. We use the new constructor to call the old constructor (using the `call` method in order to be able to give it the new object as its this value). Once we call this constructor, we can assume it requires all the same arguments as the constructor it inherits from. Finally, we can override some of the parent constructor's properties by redefining our own new ones.

We can modify the `dataTable` function to use `RTextCell` for cells who have numeric values.
...
Inheritance is fundamental to object-oriented programming, alongside encapsulation and polymorphism.But while the latter two are now generally regarded as wonderful ideas, inheritance is somewhat controversial.
