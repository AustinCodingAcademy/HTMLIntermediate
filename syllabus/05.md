...
When we are programming, these works won't all be predefined for us, so we'll often fall into the patter of the first example, working out the precise steps the program has to perform, line by line, ignorant of the higher-level concepts that they express.
...
Often, you won't pass a predefined function to forEach but create a function value on the spot instead.
...
When this happens, Javascript has the `apply` method. You can pass it an array or array-like object of arguments and it will call the fuction with those arguments.
...
Two movies were made between 1980 and 1989, "The Abyss" and "Field of Dreams".
...
// returns "Alien", "The Abyss", and "Sphere" in a new array of objects
```
...
// ["Ridley Scott", "James Cameron", "Phil Alden Robinson", "Barry Levinson"]
```
...
Often, programmers want to grab a single object from an array. For example, the earliest of the movies from our `movie_list` object.

```javascript
console.log(movie_list.reduce(function(min, cur) {
    if (cur.release_date < min.release_date) {
        return cur;
    } else {
        return min;
    }
