...
The `test` method is the simplest of regular expression tests. It tells you is there's a match and that's it. Next, let's look at the `exec` method. It returns `null` if there's no match and an informational object if there is one.
...
1. Refer to the table in the chapter summary for help. Test each solution with a few test strings.
