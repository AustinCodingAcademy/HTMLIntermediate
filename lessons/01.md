...
> We are going to talk about just what function are soon enough, I just want you to know that some exist specifically for numbers for now.
...
In EcmaScript 6 the template string was introduced. We'll cover how to use these more after we set up our computers for more modern coding.
...
We can determine the length of a string by calling its `length` propery. The length of the string in the `panagram` variable is 40.
...
> 2. Try concatenating string with some of the other data types we have explored, what happens?
